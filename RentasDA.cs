...
namespace WSPlataforma.DA
{
    ...
    {
        readonly ReprocessDA reprocc = new ReprocessDA();
        readonly WebServiceUtil WebServiceUtil = new WebServiceUtil();
        ...
        public RentasFilterTicketsResponseVM PD_REA_CLIENT_TICKETS(RentasFilterTicketsBM data)
        {
            ...
            try
            {
                ...
                while (odr.Read())
                {
                    ...
                    item.SALERT = odr["SALERT"].ToString();
                    list.Add(item);
                }
        ...
        public RentasStatusTicketTikectResponseVM PD_UPD_STATUS_TICKET(RentasStatusTicketTikectBM data)
        {
            ...
            //si P_SNAME_ACT es 'APROBAR' SE LLAMA A A PD_APROBAR
        ...
        // DEVUELVE LA RUTA DONDE SE GUARDARÁ LOS ARCHIVOS ADJUNTO DEL TICKET
        ...
        //OBTIENE LOS EJECUTIVOS RESPONSABLES
        ...
        //LISTA LA CONFIGURACION DE INFORMACION PERMITIDA PARA ADJUNTAR ARCHIVOS
        ...
        //LISTA LAS INTERFACEZ A REALIZAR DE ACUERDO A LA CONFIGURACION POR MOTIVO Y SUBMOTIVO
        public RentasListTicketInterfaceVM[] PD_LIST_TICKET_INTERFACE(RentasListTicketInterfaceBM data)
        {
            List<RentasListTicketInterfaceVM> list = new List<RentasListTicketInterfaceVM>();

            try
            {
                var sPackageName = ProcedureName.pkg_rentas + ".SP_PD_LIST_TICKET_INTERFACE";
                List<OracleParameter> parameters = new List<OracleParameter>();

                parameters.Add(new OracleParameter("P_NERRORNUM", OracleDbType.Int32, data.P_NTICKET, ParameterDirection.Input));
                ...
                while (odr.Read())
                {
                    RentasListTicketInterfaceVM item = new RentasListTicketInterfaceVM();
                    item.NINTERFACE = Convert.ToInt32(odr["NINTERFACE"]);
                    list.Add(item);
                }
        ...
        //SE LANZA AL PRECIONAR EL BOTON DE APROBAR PARA LLAMAR A SUS INTERFACEZ
        public async Task<RentasGenericResponseVM> PD_APROBAR(RentasGetMessage data)
        {
            var response = new RentasGenericResponseVM();

            var calculationData = new RentasGetMessage();
            var data2 = new RentasListTicketInterfaceBM();

            data2.P_NTICKET = 11097;


            var interfacez = PD_LIST_TICKET_INTERFACE(data2);

            foreach (var item in interfacez)
            {
                switch (item.NINTERFACE)
                {
                    //Cambiar el estado 
                    case 1:
                        var cambioEstadoResponse = await CAMBIO_ESTADO(calculationData);
                        break;

                    //Endozo de polizas
                    case 2:
                        var endosoResponse = await ENDOSO(calculationData);
                        break;
                    //Creacion de la nota de credito
                    case 3:
                        var notaCreditoResponse = await NOTA_CREDITO(calculationData);
                        break;

                    //Generenracion de la interfaz contable
                    case 4:
                        var interfazContableResponse = await INTERFAZ_CONTABLE(calculationData);
                        break;
                    //Generacion de la interfaz de tesoreria
                    case 5:
                        var interfazTesoreriaResponse = await INTERFAZ_TESORERIA(calculationData);
                        break;
                    default:
                        break;
                }
        ...
        //Cambiar el estado 
        public async Task<RentasGenericResponseVM> CAMBIO_ESTADO(RentasGetMessage data)
        {
            ...
            try
            {
                string urlServicio = "http://localhost:30897/Api/Rentas/PD_GET_MESSAGE";
                var jsonData = JsonConvert.SerializeObject(data);
                var content = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var jsonResponse = await WebServiceUtil.PostServicioRentas(urlServicio, urlServicio, "token", content);
                response = JsonConvert.DeserializeObject<RentasGenericResponseVM>(jsonResponse, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
            }
            catch (Exception ex)
            {
        ...
        //Endozo de polizas
        public async Task<RentasGenericResponseVM> ENDOSO(RentasGetMessage data)
        {
            var response = new RentasGenericResponseVM();
            try
            {
        ...
        //Creacion de la nota de credito
        public async Task<RentasGenericResponseVM> NOTA_CREDITO(RentasGetMessage data)
        {
            var response = new RentasGenericResponseVM();
            try
            {
        ...
        //Generenracion de la interfaz contable
        public async Task<RentasGenericResponseVM> INTERFAZ_CONTABLE(RentasGetMessage data)
        {
            var response = new RentasGenericResponseVM();
            try
            {
        ...
        //Generacion de la interfaz de tesoreria
        public async Task<RentasGenericResponseVM> INTERFAZ_TESORERIA(RentasGetMessage data)
        {
            var response = new RentasGenericResponseVM();
            try
            {
        ...
        public async Task<RentasGenericResponseVM> GET_CALCULATION_AMOUNT(RentasGetCalculationAmountBM data)
        {
            var response = new RentasGenericResponseVM();
            ...
            try
            {
                ...
                // Convertimos el objeto data a JSON
                var jsonData = JsonConvert.SerializeObject(data);

                // Creamos el contenido de la solicitud POST
                var content = new StringContent(jsonData, Encoding.UTF8, "application/json");

                // Llamamos al método PostServicioRentas
                var jsonResponse = await WebServiceUtil.PostServicioRentas(urlServicio, urlServicio, "token", content);

                // Deserializamos la respuesta JSON a RentasGetCalculationAmountResponseVM
                response = JsonConvert.DeserializeObject<RentasGenericResponseVM>(jsonResponse, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
            }
            catch (Exception ex)
            {
